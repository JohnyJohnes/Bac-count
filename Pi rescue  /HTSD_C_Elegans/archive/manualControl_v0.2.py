...
import pigpio
from Components import PWM

## define GPIO pins
##gpiomotor = [[14,15,18,23],[24,25,8,7],[4,17,27,22]]
gpiomotor = [[6,13,19,26],[5,22,27,17],[21,20,16,12]]
X_limit_pin = 23
Y_limit_pin = 24
Z_limit_pin = 25
...
##    mainWindow.Button.clicked.connect(lambda: myMotor.motor_go(clockwise=False, steptype="Full", steps=100, stepdelay=.01, verbose=False, initdelay=.05))
    mainWindow.xLeftButton.pressed.connect(lambda: myMotor.activate(0, 1, update = True))
    mainWindow.xLeftButton.released.connect(lambda: myMotor.deactivate(0))
    mainWindow.xRightButton.pressed.connect(lambda: myMotor.activate(0, -1, update = True))
    mainWindow.xRightButton.released.connect(lambda: myMotor.deactivate(0))
    mainWindow.yLeftButton.pressed.connect(lambda: myMotor.activate(1, 1, update = True))
    mainWindow.yLeftButton.released.connect(lambda: myMotor.deactivate(1))
    mainWindow.yRightButton.pressed.connect(lambda: myMotor.activate(1, -1, update = True))
    mainWindow.yRightButton.released.connect(lambda: myMotor.deactivate(1))
    mainWindow.zUpButton.pressed.connect(lambda: myMotor.activate(2, 1, update = True))
    mainWindow.zUpButton.released.connect(lambda: myMotor.deactivate(2))
    mainWindow.zDownButton.pressed.connect(lambda: myMotor.activate(2, -1, update = True))
    mainWindow.zDownButton.released.connect(lambda: myMotor.deactivate(2))
